...
def build_navn(comp_title: str) -> str:
    ...
    navn = re.sub(r"\b(\d{2}/\d{2})\b", lambda m: expand_year(m.group(1)), comp_title)
...
def parse_standings(url: str) -> None:
    ...
    print(
        "|dato=~~~~~ <!-- HUSK å oppdatere dato (~~~~~) dersom du har oppdatert tabellen. Fjernes ved sesongslutt. -->"
    )
    print(f"|kilde={url}")
    print(f"|navn={navn} }}}}")
